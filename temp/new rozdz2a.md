...
## 2.12. Rola stałej Eulera \(e\) w kaskadowej relaksacji węzłów

W TSM 0‑Matryca jest dyskretna (sub‑planckowskie komórki), ale makroskopowo gładka. Proces relaksacji węzła (rozpad, dyssypacja) polega na emisji kolejnych pakietów energii – fal poprzecznych (fotonów) lub lżejszych węzłów. Każdy pakiet niesie ułamek początkowej energii. Niech węzeł o początkowej energii \(E_0\) ulega kaskadzie \(n\) etapów, na każdym oddając \(\frac{1}{n}\) swojej aktualnej energii. Po \(n\) etapach pozostaje energia:

\[
E_n = E_0 \left(1 - \frac{1}{n}\right)^n.
\]

W granicy \(n \to \infty\) (nieskończenie drobne paczki) otrzymujemy:

\[
\lim_{n\to\infty} E_n = E_0 \cdot e^{-1}. \tag{2.27}
\]

Zatem **liczba \(e\) jest naturalną granicą procesu ciągłej, wykładniczej relaksacji**. Twoja intuicja odwraca perspektywę: to nie ciągłość, lecz **dyskretny proces o coraz drobniejszych krokach** prowadzi do \(e\). W TSM parametrem kontrolującym „drobność kroków” jest gęstość upakowania 0‑cząstek – im wyższa, tym więcej etapów kaskady.

### 2.12.1. Związek z wykładniczym prawem rozpadu i szeregiem Taylora

Rozwijając funkcję wykładniczą w szereg Taylora wokół \(\tau=0\) (dla małych \(\Delta\tau\)):

\[
e^{-\gamma\Delta\tau} = \sum_{k=0}^\infty \frac{(-\gamma\Delta\tau)^k}{k!}. \tag{2.28}
\]

Każdy wyraz szeregu odpowiada procesowi, w którym nastąpiło dokładnie \(k\) zdarzeń relaksacyjnych (np. emisji pakietów) w czasie \(\Delta\tau\). **Prawdopodobieństwo, że zajdzie \(k\) zdarzeń, jest proporcjonalne do \(1/k!\)** (po odpowiednim unormowaniu). Dla \(\gamma\Delta\tau = 1\) (średnio jedno zdarzenie) otrzymujemy rozkład Poissona:

\[
P(k) = \frac{1}{e \cdot k!}, \qquad \sum_{k=0}^\infty P(k) = 1, \qquad \langle k \rangle = 1. \tag{2.29}
\]

Zatem **liczba \(e\) jest stałą normalizującą dla procesów kaskadowych o jednostkowej średniej**. W TSM parametr \(\gamma\Delta\tau\) jest związany z lokalnym ciśnieniem Substratu (\(P_{\text{ext}}\)) i złożonością topologiczną węzła.

### 2.12.2. Hierarchia stanów wzbudzonych a rozwinięcie \(e\)

Dla węzła o ustalonym \(\mathcal{W}\) (np. \(\mathcal{W}=3\)) istnieją nie tylko stany orientacyjne (3 – elektron, mion, taon), ale także **stany wzbudzone o dodatkowych skrzyżowaniach** (większa liczba pętli). Energia \(n\)-tego wzbudzenia może skalować jak \(E_n \sim E_0 / n!\) (coraz mniejsze paczki). Wtedy suma energii wszystkich wzbudzeń (od \(n=0\) do \(\infty\)) dąży do:

\[
E_{\text{tot}} = E_0 \sum_{n=0}^\infty \frac{1}{n!} = E_0 \cdot e. \tag{2.30}
\]

Zatem **całkowita energia zgromadzona w kaskadzie wzbudzeń jest skończona i wyraża się przez \(e\)**. W języku teorii węzłów, liczba możliwych konfiguracji o danej złożoności topologicznej rośnie jak \(n!\), ale ich wkład energetyczny maleje tak, że suma jest zbieżna do \(e\). To tłumaczy, dlaczego w eksperymentach obserwujemy tylko skończoną liczbę stabilnych cząstek (najniższe wzbudzenia) – wyższe wzbudzenia są niezwykle krótkotrwałe (czas życia \(\tau_n \sim 1/(n!)\)) i rozpadają się natychmiast, ale ich ślad (jako rezonanse o szerokości \(\Gamma_n \sim n!\) w skali energii) może być rejestrowany przy wystarczająco wysokiej rozdzielczości.

### 2.12.3. Wzór na średnią liczbę produktów rozpadu

Dla procesu kaskadowego, w którym każdy etap produkuje średnio \(m\) nowych cząstek (lub pakietów falowych), a prawdopodobieństwo kontynuacji po \(n\) etapach maleje jak \(1/n\) (co jest typowe dla procesów rozgałęzionych z tłumieniem), całkowita średnia liczba produktów wynosi:

\[
\langle N \rangle = \sum_{n=1}^\infty \frac{m}{n!} = m(e-1). \tag{2.31}
\]

- Dla \(m=1\) (proces liniowy, np. emisja pojedynczego fotonu w każdym kroku) \(\langle N \rangle = e-1 \approx 1.718\). To może odpowiadać średniej liczbie fotonów w kaskadzie elektromagnetycznej przy niskich energiach.
- Dla rozpadu neutronu (na proton, elektron i antyneutrino) mamy \(m=3\), ale nie jest to proces liniowy – to pojedynczy etap. Jednak dla kaskad hadronowych w zderzeniach wysokoenergetycznych, gdzie produkowane są dżety, \(m\) może być bliskie 2 (kwark antykwark), co daje \(\langle N \rangle \approx 3.436\) – zgodne z typową krotnością w dżetach lekkich.

### 2.12.4. Implikacje dla TSM i falsyfikowalne przewidywania

1. **Stała \(e\) jest podstawową stałą Substratu**, podobnie jak \(\pi\) (geometria sfery) i \(\sqrt{2}\) (reguła Koidego). Jej pojawienie się w granicy ciągu \(\left(1+\frac{1}{n}\right)^n\) odzwierciedla **dyskretno‑ciągły charakter 0‑Matrycy**: im wyższa gęstość upakowania \(\phi\) (większe \(n\)), tym gładszy (bardziej ciągły) proces relaksacji, a granicą jest właśnie \(e\).

2. **Hierarchia wzbudzeń** (dodatkowe pętle, skrzyżowania) naturalnie generuje szereg \(\sum 1/n!\), co wyjaśnia, dlaczego obserwujemy tylko kilka stabilnych stanów (np. trzy leptony) – wyższe wzbudzenia są niezwykle krótkotrwałe, a ich wkład energetyczny jest zaniedbywalny w eksperymentach przy obecnych rozdzielczościach.

3. **Procesy relaksacyjne** (rozpad neutronu, anihilacja, promieniowanie) podlegają statystyce Poissona z parametrem \(\lambda = \gamma\tau\). W TSM parametr \(\gamma\) zależy od lokalnego ciśnienia Substratu \(P_{\text{ext}}\) oraz od złożoności topologicznej węzła (\(\chi_{\text{top}}\)). Dla neutronu w butelce (wyższe \(P_{\text{ext}}\)) \(\gamma\) jest większe, co tłumaczy anomalię czasu życia.

**Falsyfikowalne przewidywanie:** W zderzeniach wysokoenergetycznych (np. \(e^+e^- \to hadrony\)), rozkład liczby produktów w kaskadach elektromagnetycznych powinien być zgodny z rozkładem Poissona o średniej \(\langle k \rangle = 1\) (lub ogólnie \(m(e-1)\)). Odstępstwa od tego rozkładu (np. rozkład logarytmicznie normalny lub potęgowy) mogą być wskaźnikiem nowej fizyki (np. przejścia w reżim silnie nieliniowy, gdzie dominują oddziaływania silne).

---

## 2.13. Podsumowanie rozdziału 2 – od węzła do cząstki
...
### 2.13.1. Kluczowe osiągnięcia formalizmu TSM
...
| **Rola stałej \(e\)** | Granica kaskady dyskretnych pakietów; normalizacja rozkładu Poissona; suma energii wzbudzeń (2.30). | 2.12 |
...
### 2.13.2. Tabela przewidywanych stanów dla różnych \(\mathcal{W}\)
...
### 2.13.3. Nowe, falsyfikowalne przewidywania TSM
...
*Koniec rozdziału 2 (wersja 3.3.0)*