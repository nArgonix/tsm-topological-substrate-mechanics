...
Równania ze źródłami mają inny status. Równanie Naviera‑Cauchy'ego, po wyodrębnieniu antysymetrycznej części wirowej, opisuje jak rozkład naprężeń wewnątrz 0-Matrycy generuje gradienty pola. Utożsamiając 4-dywergencję tensora $F^{\mu\nu}$ z 4-prądem torsji $J^\mu$ (postulat identyfikacyjny: gęstość topololgicznego skrętu = ładunek elektryczny, strumień skrętu = prąd), uzyskujemy:
...
Równanie (3.2.4) w rozwinięciu składowym odtwarza Prawo Gaussa ($\mu = 0$) oraz Prawo Ampère'a‑Maxwella ($\mu = i$). Jest to **postulat identyfikacyjny TSM**: dynamika Naviera‑Cauchy'ego generuje strukturę tensoriową $\partial_\nu F^{\mu\nu}$, a jej źródłem jest topologiczny prąd węzłów. Pełne wyprowadzenie członu źródłowego z pierwszych zasad mechaniki 0-Matrycy jest przedmiotem odrębnej analizy.
