<!-- ver:4.7.1-->
# Elasto-mechanika 0-Matrycy: Emergencja Czasu, Metryki i Kinematyki Relatywistycznej {#sec:rom-tsm}

## Wprowadzenie i ramy konceptualne {#sub-wprowadzenie}
...
## Mikrostruktura 0-Matrycy i stan globalnego zakleszczenia {#sec-1-2}
...
### Geometria sieci: 0-Cząstki w sferach oscylacji jako fundament osnowy {#sec-1-2-1}
...
* **Rdzeń** stanowi fizyczną, nieprzenikalną i fundamentalnie niepodzielną bryłę o objętości $V_{00}$. Nie posiada on żadnej struktury wewnętrznej, a jego objętość nie ulega jakiejkolwiek kompresji nawet w warunkach ekstremalnego naporu. Posiada także niezmienniczą masę $m_{00}$ oraz szybkość $v_{00}=\text{const}$. Te fundamentalne własności mikroskopowe, identyczne dla wszystkich 0-Cząstek i niezależne od stanu makroskopowego osnowy. Objętość $V_{00}$, masa $m_{00}$ oraz $v_{00}$ są **aksjomatem pierwotnym** modelu TSM: nie wyłaniają się z geometrii sieci ani z dynamiki kolizji, lecz stanowią ontologicznie pierwotną wielkość definiującą skalę energetyczną 0-Matrycy, analogicznie jak ładunek elementarny w elektrodynamice.
...
#### Mikrodynamika komórki: Mechanizm bilardu Sinaja {#sec-1-2-1-1}
...
#### Emergencja makroskopowej tarczy sferycznej {#sec-1-2-1-2}
...
#### Reżim krytyczny i Klauzula Krytycznego Przemieszczenia {#sec-1-2-1-3}
...
### Mechanizm globalnego zakleszczenia i wyłonienie izotropowej 3‑brany {#sec-1-2-2}
...
#### Matematyczny formalizm uśredniania – Efektywna Gęstość Substratu {#sec-1-2a-2}
...
#### Kanoniczna definicja czasu makroskopowego jako miary sukcesji stanów sieci {#sec-1-2-3}
...
* $dt$ – przyrost lokalnego czasu własnego, wyrażony w jednostkach pomiarowych (sekundach).
* dN$ – czysta liczba zaobserwowanych zdarzeń topologicznych. Jest to fundamentalny wskaźnik aktywności układu, zdefiniowany jako suma zrealizowanych pełnych cykli fazowych (np. obrotów spinowych lub oscylacji solitonowych).
* $T_0$ – stała kalibracyjna (współczynnik translacji). Nie posiada statusu bytu fundamentalnego, jest „tłumaczem” metrologicznym, pozwalającym na zmapowanie obiektywnej liczby cykli ($dN$) na standardowe jednostki czasu przyjęte w fizyce makroskopowej.
* $\phi_0$ – podstawowa, izotropowa gęstość upakowania 0-Matrycy w Stanie Zero (przestrzeń płaska).
* $\langle\phi(\mathbf{x})\rangle_{\text{macro}}$ – uśredniona, lokalna gęstość osnowy w obszarze zajmowanym przez układ.
...
## Elastodynamika sieciowa i mikroskopowa geneza prędkości światła {#sec-1-3}
...
### Propagacja zaburzeń ścinania i blokada fal podłużnych w osnowie 4D {#sec-1-3-1}
...
Jako 4-wymiarowy, izotropowy ośrodek sprężysty o gigantycznym zagęszczeniu kolizyjnym, 0-Matryca podlega uogólnionym równaniom Naviera-Cauchy'ego dla przemieszczeń $\mathbf{u}(x^1, x^2, x^3, x^4)$. Przyjmujemy tu i w dalszej części rozdziału następującą konwencję notacyjną: indeksy łacińskie ze środka alfabetu ($i,j,k,\dots$) oraz wielkie ($I,J$, wprowadzone w @sec-1-2-2) oznaczają zamiennie współrzędne pełnej, 4-wymiarowej przestrzeni euklidesowej Substratu i przebiegają zakres $\{1,2,3,4\}$; natomiast indeksy $a,b,c,d$, używane od @sec-1-4 wzwyż, są zarezerwowane wyłącznie dla 3-wymiarowej hiperpowierzchni 3-brany ($\{1,2,3\}$). Wprowadzamy 4-wymiarowy tensor odkształceń Cauchy'ego-Greena $\epsilon_{ij}$ w postaci:
...
Zakładając liniowy reżim sprężysty (w którym nieliniowe gradienty kwadratowe są pomijalne przed przekroczeniem progu blokady topologicznej), gęstość energii sprężystej $\mathcal{U}$ dana jest klasycznym wzorem Lamégo rozszerzonym do 4 wymiarów (przy zastosowaniu konwencji sumacyjnej Einsteina dla powtarzających się indeksów):
...
Równanie ruchu elementu objętości osnowy w czasie płaskiego tła $t_{\text{flat}}$ można zapisać w postaci wektorowej, gdzie operatory różniczkowe $\nabla$, $\nabla \cdot$ oraz $\nabla^2$ działają w pełnej przestrzeni $\mathbb{R}^4$, a $\rho_{\text{eff}}$ oznacza efektywną makroskopową gęstość falową:
...
Zgodnie z uogólnionym twierdzeniem Helmholtza w przestrzeniach wielowymiarowych, rozkładamy pole przemieszczeń na część bezrotacyjną (podłużną) $\mathbf{u}_\parallel$ oraz bezźródłową (poprzeczną) $\mathbf{u}_\perp$:

$$\mathbf{u} = \mathbf{u}_\parallel + \mathbf{u}_\perp, \quad \text{gdzie} \quad \frac{\partial u_j^\parallel}{\partial x^i} - \frac{\partial u_i^\parallel}{\partial x^j} = 0 \quad \text{oraz} \quad \frac{\partial u_i^\perp}{\partial x^i} = 0$$ {#eq-sek131-pole-wekt}

Warunek lokalnej nieściśliwości osnowy całkowicie eliminuje fale podłużne z fizycznego spektrum makroskopowego. Równanie Naviera-Cauchy'ego redukuje się wyłącznie do poprzecznej składowej $\mathbf{u}_\perp$, reprezentującej lokalne ścinanie (rotację sfer oddziaływań):
...
Ponieważ 3-brana jest zdefiniowana jako makroskopowe skręcenie uwięzione wzdłuż czwartego wymiaru ($x^4$), wszelkie interakcje w naszym świecie są falami poprzecznymi tej struktury. Rozpatrując, bez straty ogólności, wektor falowy zorientowany w płaszczyźnie $(x^1, x^4)$, czyli $\mathbf{k} = (k_1, 0, 0, k_4)$, otrzymujemy relację dyspersyjną $\omega^2 = c_{\perp}^2 (k_1^2 + k_4^2)$. Maksymalna prędkość rzutu fali poprzecznej na obserwowalną przestrzeń 3D zachodzi dla składowej uwięzionej $k_4 = 0$ i wyznacza absolutną granicę prędkości zjawisk fizycznych. Prędkość $c_{\perp}$ jest zatem tożsama z makroskopową prędkością światła $c$ w próżni.

### Dynamiczny opór relaksacyjny i mikroskopowe ugruntowanie parametrów {#sec-1-3-2}
...
Wprowadźmy w tym miejscu stałą sieci osnowy $a$ — odległość między równowagowymi położeniami sąsiednich węzłów $\mathbf{X}_i$. Wielkość ta nie jest niezależnym parametrem modelu, lecz wynika wprost z geometrii sfer oscylacji wprowadzonych w [@sec-1-2-1] w stanie pełnej relaksacji zakleszczonej sieci sąsiadujące sfery oddziaływań o promieniu $R_0$ stykają się stycznie, definiując graniczny kontakt komórek konfiguracyjnych [@sec-1-2-1-1]. Dla izotropowej sieci o identycznych sferach oscylacji odległość międzywęzłowa równa jest zatem sumie promieni dwóch sąsiednich komórek:
...
Zdefiniujmy bezwymiarowy stosunek $\xi$ między stałą sieci osnowy $a$ a długością fali mierzalnego zaburzenia makroskopowego $\lambda_\text{fala}$:
...
Definiujemy makroskopowy parametr geometrycznego wzmocnienia bezwładności $\Gamma_{4D}$, reprezentujący liczbę sub-planckowskich mikro-cykli kolizyjnych niezbędnych do wykonania makroskopowego, w pełni zachowawczego przekazu impulsu (por. [@sec-1-3-3]).

### Derywacja $\Gamma_{4D}$ z błądzenia przypadkowego rdzenia {#sec-1-3-3}
...
### Derywacja $c_{\perp}$ i jej interpretacja fizyczna {#sec-1-3-4}

Podstawiając niezależnie wyprowadzone $\mu=\rho_0v_\text{00}^2/4$ (@sec-1-3-2) oraz $\rho_{\text{eff}}^{(0)}=\rho_0\Gamma_{4D}^2$ ( @eq-1-3-3-wz5, @sec-1-3-3) do redukcji Helmholtza $c_\perp=\sqrt{\mu/\rho_{\text{eff}}^{(0)}}$:
...
### Zachowawczy charakter propagacji falowej i brak dyssypacji energii {#sec-1-3-5}
...
Całkowity bilans energetyczny transferu pędu w jednym pełnym cyklu falowym wynosi dokładnie zero. Dlatego też redshift obserwowany w kosmologii nie jest efektem tarcia fotonu o osnowę (co odróżnia TSM od modeli „zmęczonego światła"), lecz wynika z kosmologicznej ewolucji uśrednionej gęstości substratu $\langle\phi\rangle_\text{macro}$ — poprzez mechanizm wstępni opisany w Rozdziale 0.4 oraz dokładniej w Rozdziale 6. A lokalna zmiana $\langle\phi\rangle_\text{macro}$ modyfikuje tempo kanonicznego czasu (@eq-1-2-5), co obserwator rejestruje jako zmianę częstotliwości fali.

### Status fali podłużnej: Superluminalne fale gęstości 4D i fluktuacje czasu {#sec-1-3-6}
...
## Kinematyka nieliniowa i emergencja efektywnej geometrii makroskopowej {#sec-1-4}
...
* $\mathbf{u}$ – wektor fizycznego przemieszczenia osnowy Substratu,
...
### Kinematyka fali nieliniowej i fizyczna kontrakcja przestrzeni {#sec-1-4-2}
...
#### Konstrukcja prędkości i kontrakcja długości {#sec-1-4-2-1}
...
Pod wpływem tego jednostronnego oporu elasto-mechanicznego, kształt każdego węzła solitonowego ulega rzeczywistej kompresji mechanicznej , co makroskopowo manifestuje się jako skrócenie długości w kierunku ruchu:

$$L' = L / \gamma.$$ {#eq-1-4-6a}


### Kinematyczna dylatacja czasu jako izotropowe spiętrzenie osnowy {#sec-1-4-3}
...
Mechanizm tego spiętrzenia wynika bezpośrednio z zasady zachowania objętości rdzenia 0-Cząstki ($V_{00}=\text{const}$, @sec-1-2-1) w połączeniu z kontrakcją przestrzenną struktury solitonowej w kierunku ruchu. Jak wykazano niezależnie w @sec-1-4-2-1  z wymogu niezmienniczości kształtu solitonu względem jednorodnego równania falowego osnowy, poruszający się węzeł ulega skróceniu rozciągłości przestrzennej wzdłuż kierunku ruchu o czynnik $1/\gamma$: $L'=L/\gamma$ (@eq-1-4-6a). Ponieważ liczba 0-Cząstek $N$ tworzących strukturę solitonu oraz objętość ich pojedynczych, niepodzielnych rdzeni $V_{00}$ pozostają niezmienione (absolutna sztywność rdzenia, @sec-1-2-1-1), redukcja objętości przestrzennej zajmowanej przez tę samą liczbę cząstek wymusza proporcjonalny wzrost lokalnej gęstości upakowania:
...
#### Dylatacja grawitacyjna — związek z potencjałem $\Phi$ {#sec-1-4-3-1}
...
### Energia, pęd i masa relatywistyczna {#sec-1-4-4}
 ...
 *(Powyższe przejście zakłada ruch jednowymiarowy wzdłuż kierunku propagacji solitonu, zgodnie z konwencją całego @sec-1-4-4 - @sec-1-4-5; dla składowej siły prostopadłej do $v$ czynnik skalujący wynosiłby $\gamma^1$, nie $\gamma^3$.)*
...
### Stałe materiałowe osnowy a lokalna zmienność propagacji fal ($c_{\perp}$) {#sec-1-4-5}
...
**Wkład pola magnetycznego.** Naprężenie Maxwella generowane przez pole $B$ w ośrodku wynosi $\Sigma_B = B^2/(2\mu_\text{em})$, gdzie $\mu_\text{em}$ oznacza przenikalność magnetyczną próżni (symbol wyróżniony od modułu ścinania Lamégo $\mu$ stosowanego w całym rozdziale). Podstawiając do ( @eq-zmiennosc-c):

$$c_{\perp,\text{lok}}^2 \approx c_{\perp}^2\left(1 + \mathcal{A}\frac{B^2}{2\mu_{\text{em}}}\right)$$ {#eq-1-4-5-wz1}

**Wkład potencjału grawitacyjnego.** Z relacji gęstości @sec-1-4-3 (@eq-1-4-2), $\phi(\mathbf{x})\approx\phi_0(1+|\Phi|/c_\perp^2)$, oraz mostu gęstości $\rho_{\text{eff}}=\rho_0\cdot\phi/\phi_0$ wprowadzonego i formalnie uzasadnionego w @sec-1-5-1 (@eq-1-5-5b), otrzymujemy $\rho_{\text{eff}}(\mathbf{x})\approx\rho_0(1+|\Phi|/c_\perp^2)$. Ponieważ $c_{\perp,\text{lok}}^2=\mu/\rho_{\text{eff}}$, rozwinięcie do pierwszego rzędu daje:
...
### Wnioski końcowe i synteza kinematyki TSM {#sec-1-4-6}
...
## Dynamika falowa Substratu i podwójny reżim elasto‑dynamiki {#sec-1-5}
...
### Równania pola osnowy jako opis stanów naprężeń sieci {#sec-1-5-1}
...
gdzie pierwszy człon opisuje gęstość energii kinetycznej pola przemieszczeń, a drugi — gęstość energii potencjalnej sprężystej $\mathcal{U}$. Tensor małych odkształceń $\epsilon_{ab}$ wyznacza się bezpośrednio z gradientów pola przemieszczeń osnowy $u$:

$$\epsilon_{ab} = \frac{1}{2}(\partial_a u_b + \partial_b u_a)$$  {#eq-1-5-2}

Właściwości makroskopowe zawarte w tensorze $K^{abcd}$ oraz determinowany przez nie tensor naprężeń $\sigma^{ab}$ wynikają bezpośrednio ze stanu elasto-mechanicznego uwięzionych, sub-planckowskich 0-Cząstek i ich sfer odpychania. Dla idealnie izotropowej sieci krystalicznej, tensor modułów sprężystości redukuje się do dwóch niezależnych stałych Lamégo ($\lambda$ oraz $\mu$):

$$K^{abcd} = \lambda \delta^{ab}\delta^{cd} + \mu (\delta^{ac}\delta^{bd} + \delta^{ad}\delta^{bc})$$  {#eq-1-5-3}

gdzie $\delta^{ab}$ oznacza płaską, euklidesową metrykę tła Substratu. Tensor naprężeń osnowy $\sigma^{ab}$ w ujęciu konstytutywnym jest definiowany bezpośrednio przez pochodną gęstości energii sprężystej $\mathcal{U}$ względem tensora odkształcenia (co przy założeniu braku jawnej zależności energii kinetycznej od odkształceń przestrzennych w reżimie liniowym odpowiada relacji $-\partial \mathcal{L} / \partial \epsilon_{ab}$):
...
Pole $\phi(\mathbf{x})$, wprowadzone fenomenologicznie w @sec-1-2a-2 jako splot mikroskopowego stopnia upakowania sieci z oknem filtracyjnym $W$ (@eq-1-2-1), utożsamiamy w granicy continuum elastycznego z polem zdefiniowanym wprost przez lokalną dylatację objętościową ośrodka, przyjmując liniową relację $\phi(\mathbf{x}) = \phi_0(1 - \epsilon^c_c)$. To utożsamienie — analogiczne do standardowego związku między mikroskopową gęstością liczbową a makroskopową dywergencją przemieszczenia w klasycznej teorii sprężystości ośrodków ziarnistych — stanowi założenie konstytutywne modelu TSM, a nie twierdzenie wyprowadzone wprost z mikrodynamiki bilardu Sinaja (@sec-1-2-1-1); jego ścisły dowód pozostaje otwartym problemem badawczym tego rozdziału, na równi z derywacją $c_\perp$ (@sec-1-4-5) i strukturą radialną solitonu (@sec-1-6-1).

*Uwaga o hierarchii progów:* W TSM obowiązuje ścisłe uporządkowanie $\phi_0 \le \phi_c < \phi_\text{lock}$. Próg $\phi_c$ wyznacza globalny stan zakleszczenia sieci będący tłem fizycznym; próg $\phi_\text{lock}$ jest lokalnym kryterium tworzenia trwałych węzłów topologicznych (materii) i zawsze przekracza $\phi_c$. Ilościowa relacja między tymi progami — oraz ich zależność od wymiaru przestrzeni i geometrii rdzeni — jest otwartym problemem ilościowym modelu.
...
Stosując zasadę najmniejszego działania Hamiltona ($\delta \int \mathcal{L} \, d^4x = 0$) względem pola przemieszczeń Substratu $u_i$, otrzymujemy dynamiczne równanie ruchu 4-wymiarowej osnowy (odpowiednik równania Naviera-Cauchy’ego w środowisku ciągłym):

$$\rho_{\text{eff}} \frac{\partial^2 u^i}{\partial t_{\text{flat}}^2} = \nabla_j \sigma^{ij} = (\lambda + \mu) \nabla^i (\nabla \cdot \mathbf{u}) + \mu \nabla^2 u^i$$ {#eq-1-5-6}

Zgodnie z mechaniką ośrodków krystalicznych, pole przemieszczeń $u^a$ można jednoznacznie rozłożyć (dekompozycja Helmholtza) na składową bezrotacyjną (fale podłużne) oraz bezdywergencyjną (fale poprzeczne ścinania): $\mathbf{u} = \nabla \varphi_H + \nabla \times \mathbf{\Psi}$, gdzie $\varphi_H$ jest skalarnym potencjałem dekompozycji (symbol wyróżniony od grawitacyjnego $\Phi$ z @sec-1-4-3-1). Podstawiając ten rozkład do równania ruchu (@eq-1-5-6) dla składowej poprzecznej $\mathbf{\Psi}$ (gdzie $\nabla \cdot \mathbf{\Psi} = 0$), otrzymujemy czyste, jednorodne równanie falowe:
...
z makroskopową podatnością elastyczną Substratu jednoznacznie wyrażoną przez moduł ścinania: $\chi \equiv -1/\mu$. *(Znak współczynnika $\chi$ zależy od przyjętej konwencji znaku naprężenia $\sigma_{ab}$ — w tym tekście konwencja $T^{(0)}_{IJ}=-P_0\delta_{IJ}$ z @sec-1-2-2 implikuje, że naprężenie kompresyjne jest ujemne; w tej konwencji $\chi=-1/\mu$ daje fizycznie poprawny kierunek: kompresja zwiększa $g_{ab}$. Jeśli przyjmujesz odwrotną konwencję znaku $\sigma_{ab}$ w @sec-1-3, zweryfikuj spójność znaku tutaj.)* W ten sposób grawitacja wyłania się jako liniowa aproksymacja tego samego pola naprężeń, które w pełnej, nieliniowej postaci opisuje @eq-1-4-1 — eliminując potrzebę zakrzywiania samej pierwotnej ontologii przestrzeni.

### Fale sprężyste a nieliniowa blokada topologiczna {#sec-1-5-2}
...
#### **Reżim I: Liniowa relaksacja akustyczna ($\phi < \phi_{\text{lock}}$)** {#sec-1-5-2-1}
...
### Podsumowanie dynamicznego dualizmu Substratu {#sec-1-5-3}
...
## Aksjomat materii: Matematyczna struktura defektów, inercja i spin {#sec-1-6}
...
### Matematyczna struktura ładunku topologicznego i profilu solitonu {#sec-1-6-1}
...
### Elastodynamiczny opór jako źródło bezwładności {#sec-1-6-2}
...
### Wyjaśnienie metody geometryczno-falowej {#sec-1-6-3}
...
### Geometryczne pochodzenie spinu 1/2 i ontologia antymaterii {#sec-1-6-4}
